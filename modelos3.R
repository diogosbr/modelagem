modelos = function(coord, abio, k = 3, diretorio = "teste", plot = T, bc = T, mx = F, GLM = F, RF = F, 
                   SVM = F, dm = F, mah = F, proj, buffer, geo.filt = T, br, mod = 'before', tss, bootstrap = F , nrep) {
    ...
    if(bootstrap==T){
      ...
      for(aa in 1:nrep){
        ...
        for (i in 1:k) {
          ...
          if(missing(tss)){
            writeRaster(bc.mod, paste0("./modelos/", "bc_", i, "rep_",a,"_con.tif"), format = "GTiff", 
                        overwrite = T)
            writeRaster(bc.mod > tr, paste0("./modelos/bin/", "bc_", i, "rep_",aa, "_bin.tif"), format = "GTiff", 
                        overwrite = T)
            png(paste0("./png/", "bc_", i, "con.png"))
            plot(bc.mod, main = paste0("Bioclim con part_", i,"rep_",aa))
            dev.off()
            png(paste0("./png/", "bc_", i, "bin.png"))
            plot(bc.mod > tr, main = paste0("Bioclim bin part_", i,"rep_",aa))
            dev.off()
          }
          if(missing(tss)==FALSE){
            if(TSS.calc>tss){
              writeRaster(bc.mod, paste0("./modelos/", "bc_", i, "rep_",aa, "_con.tif"), format = "GTiff", 
                          overwrite = T)
              writeRaster(bc.mod > tr, paste0("./modelos/bin/", "bc_", i, "rep_",aa, "_bin.tif"), format = "GTiff", 
                          overwrite = T)
              png(paste0("./png/", "bc_", i, "con.png"))
    ...
    if(bootstrap==T){
      ...
      for(aa in 1:nrep){
        ...
        for (i in 1:k) {
          ...
          if(missing(tss)){
            writeRaster(mx.mod, paste0("./modelos/", "Maxent_", i, "rep_",aa ,"_con.tif"), format = "GTiff", 
                        overwrite = T)
            writeRaster(mx.mod > tr, paste0("./modelos/bin/", "Maxent_", i, "rep_",aa, "_bin.tif"), format = "GTiff", 
                        overwrite = T)
            png(paste0("./png/", "Maxent_", i, "rep_",aa, "con.png"))
            plot(mx.mod, main = paste0("Maxent con part_", i, "rep_",aa))
            dev.off()
            png(paste0("./png/", "Maxent_", i, "rep_",aa, "bin.png"))
            plot(mx.mod > tr, main = paste0("Maxent bin part_", i, "rep_",aa))
            dev.off()
          }
          if(missing(tss)==FALSE){
            if(TSS.calc>tss){
    ...
    if(bootstrap==T){
      ...
      for(aa in 1:nrep){
        ...
        for (i in 1:k) {
          ...
          if(missing(tss)){
            writeRaster(dm.mod, paste0("./modelos/", "dm_", i, "rep_", aa, "_con.tif"), format = "GTiff", 
                        overwrite = T)
            writeRaster(dm.mod > tr, paste0("./modelos/bin/", "dm_", i, "rep_", aa, "_bin.tif"), format = "GTiff", 
                        overwrite = T)
            png(paste0("./png/", "dm_", i, "rep_", aa, "con.png"))
            plot(dm.mod, main = paste0("Domain con part_", i, "rep_", aa))
            dev.off()
            png(paste0("./png/", "dm_", i, "rep_", aa, "bin.png"))
            plot(dm.mod > tr, main = paste0("Domain bin part_", i, "rep_", aa))
            dev.off()
          }
          if(missing(tss)==FALSE){
            if(TSS.calc>tss){
    ...
    if(bootstrap==T){
      ...
      for(aa in 1:nrep){
        ...
        for (i in 1:k) {
          ...
          if(missing(tss)){
            writeRaster(mah.mod, paste0("./modelos/", "mah_", i, "rep_", aa, "_con.tif"), format = "GTiff", 
                        overwrite = T)
            writeRaster(mah.mod > tr, paste0("./modelos/bin/", "mah_", i, "rep_", aa, "_bin.tif"), format = "GTiff", 
                        overwrite = T)
            png(paste0("./png/", "mah_", i, "rep_", aa, "con.png"))
            plot(mah.mod, main = paste0("Mahalanobis con part_", i, "rep_", aa))
            dev.off()
            png(paste0("./png/", "mah_", i, "rep_", aa, "bin.png"))
            plot(mah.mod > tr, main = paste0("Mahalanobis bin part_", i, "rep_", aa))
            dev.off()
          }
          if(missing(tss)==FALSE){
            if(TSS.calc>tss){
    ...
    if(bootstrap==T){
      ...
      for(aa in 1:nrep){
        ...
        for (i in 1:k) {
          ...
          if(missing(tss)){
            writeRaster(GLM.mod, paste0("./modelos/", "GLM_", i,"rep_", aa, "_con.tif"), format = "GTiff", 
                        overwrite = T)
            writeRaster(GLM.mod > tr, paste0("./modelos/bin/", "GLM_", i,"rep_", aa, "_bin.tif"), format = "GTiff", 
                        overwrite = T)
            png(paste0("./png/", "GLM_", i,"rep_", aa, "con.png"))
            plot(GLM.mod, main = paste0("GLM con part_", i,"rep_", aa))
            dev.off()
            png(paste0("./png/", "GLM_", i, "bin.png"))
            plot(GLM.mod > tr, main = paste0("GLM bin part_", i,"rep_", aa))
            dev.off()
          }
          if(missing(tss)==FALSE){
            if(TSS.calc>tss){
              ...
              plot(GLM.mod, main = paste0("GLM con part_", i,"rep_", aa))
              dev.off()
              png(paste0("./png/", "GLM_", i,"rep_", aa, "bin.png"))
              plot(GLM.mod > tr, main = paste0("GLM bin part_", i,"rep_", aa))
              dev.off()
            }
          }
    ...
    if(bootstrap==T){
      ...
      for(aa in 1:nrep){
        ...
        for (i in 1:k) {
          ...
          if(missing(tss)){
            writeRaster(RF.mod, paste0("./modelos/", "RF_", i, "rep_", aa, "_con.tif"), format = "GTiff", 
                        overwrite = T)
            writeRaster(RF.mod > tr, paste0("./modelos/bin/", "RF_", i, "rep_", aa, "_bin.tif"), format = "GTiff", 
                        overwrite = T)
            png(paste0("./png/", "RF_", i, "rep_", aa, "con.png"))
            plot(RF.mod, main = paste0("RF con part_", i))
            dev.off()
            png(paste0("./png/", "RF_", i, "rep_", aa, "bin.png"))
            plot(RF.mod > tr, main = paste0("RF bin part_", i, "rep_", aa))
            dev.off()
          }
          if(missing(tss)==FALSE){
            if(TSS.calc>tss){
              ...
              writeRaster(RF.mod > tr, paste0("./modelos/bin/", "RF_", i, "rep_", aa, "_bin.tif"), format = "GTiff", 
                          overwrite = T)
              png(paste0("./png/", "RF_", i, "rep_", aa, "con.png"))
              plot(RF.mod, main = paste0("RF con part_", i, "rep_", aa))
              dev.off()
              png(paste0("./png/", "RF_", i, "rep_", aa, "bin.png"))
              plot(RF.mod > tr, main = paste0("RF bin part_", i, "rep_", aa))
              dev.off()
    ...
    if(bootstrap==T){
      ...
      for(aa in 1:nrep){
        ...
        for (i in 1:k) {
          ...
          if(missing(tss)){
            writeRaster(SVM.mod, paste0("./modelos/", "SVM_", i, "rep_", aa, "_con.tif"), format = "GTiff", 
                        overwrite = T)
            writeRaster(SVM.mod > tr, paste0("./modelos/bin/", "SVM_", i, "rep_", aa, "_bin.tif"), format = "GTiff", 
                        overwrite = T)
            png(paste0("./png/", "SVM_", i, "rep_", aa, "con.png"))
            plot(SVM.mod, main = paste0("SVM con part_", i, "rep_", aa))
            dev.off()
            png(paste0("./png/", "SVM_", i, "rep_", aa, "bin.png"))
            plot(SVM.mod > tr, main = paste0("SVM bin part_", i, "rep_", aa))
            dev.off()
          }
