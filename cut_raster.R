...
cut.raster = function(raster.dir, shape.dir, extension = ".asc", plot = TRUE, trim = FALSE, 
    teste = FALSE) {
    
    require(raster)
    # original=getwd()
    
    if (dir.exists("Mask_temp") == F) {
        dir.create("Mask_temp")
    }
    if (dir.exists("Mask_temp2") == F) {
        dir.create("Mask_temp2")
    }
    ...
    # shape= maptools::readShapeSpatial (shape.dir) subsetar
    ...
    if (trim == F) {
        ...
        for (i in 1:length(names(predictors))) {
            ini1 = Sys.time()
            crop(predictors[[i]], extent(shape), filename = paste("./Mask_temp", "/", names(predictors)[i], 
                sep = "", extension), overwrite = TRUE)
            croped = list.files("./Mask_temp", pattern = extension, full.names = TRUE)
            mask(raster(croped[i]), shape, filename = paste("./Cortados", "/", names(predictors)[i], 
                sep = "", extension), overwrite = TRUE)
    ...
    if (trim == T) {
        ...
        for (i in 1:length(names(predictors))) {
            ...
            crop(predictors[[i]], extent(shape), filename = paste(".\\Mask_temp", "\\", names(predictors)[i], 
                sep = "", extension), overwrite = TRUE)
            croped = list.files(".\\Mask_temp", pattern = extension, full.names = TRUE)
            mask(raster(croped[i]), shape, filename = paste(".\\Mask_temp2", "\\", names(predictors)[i], 
                sep = "", extension), overwrite = TRUE)
            
            writeRaster(trim(raster(list.files(".\\Mask_temp2", pattern = extension, full.names = TRUE)[i])), 
